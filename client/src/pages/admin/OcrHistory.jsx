...
import { ScanLine, Loader, CheckCircle, AlertTriangle, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 md:py-8 space-y-6">
        ...
        {/* History List */}
        {loading ? (
          <div className="flex justify-center py-20"><Loader size={32} className="animate-spin text-blue-600" /></div>
        ) : scans.length === 0 ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {scans.map((scan, i) => (
              <div key={scan._id} className="border-b border-gray-50 last:border-0">
                <div onClick={() => setExpanded(expanded === i ? null : i)} className="flex items-start gap-4 px-4 md:px-6 py-4 hover:bg-gray-50 transition cursor-pointer">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-1 ${scan.isFake ? "bg-red-50" : "bg-green-50"}`}>
                    {scan.isFake ? <AlertTriangle size={18} className="text-red-500" /> : <CheckCircle size={18} className="text-green-500" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                      <p className="font-bold text-gray-800 truncate">{scan.medicineName || "Unknown Item"}</p>
                      <span className={`w-fit text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${scan.isFake ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
                  ...
                  <div className="mt-2 text-gray-400">{expanded === i ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</div>
                </div>
        ...
        {pages > 1 && (
          <div className="flex items-center justify-center gap-4 py-4">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-5 py-2 text-sm bg-white border rounded-xl disabled:opacity-40 shadow-sm">Prev</button>
            <span className="text-sm text-gray-500 font-medium">Page {page} of {pages}</span>
            <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages} className="px-5 py-2 text-sm bg-white border rounded-xl disabled:opacity-40 shadow-sm">Next</button>
          </div>
        )}
      </div>
    </div>
